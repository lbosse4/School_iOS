...
/*
        ...
        for aView in pentominoImageViews {
            ...
            self.weirdrotatePentominoView(aView, numRotations: rotationsNeededToReturn, width: &pieceWidth, height: &pieceHeight, isSolve: self.isReset, directionFactor: 1.0)
            ...
            if flipSolution != 0 {
                ...
                if evenOrOdd == self.isOdd{
                    self.weirdflipPentominoView(aView, numRotations: rotationSolution, x: positiveTransformValue, y: negativeTransformValue)
                }else {
...
//                //
//                //                if evenOrOdd == self.isOdd{
//                self.flipPentominoView(aView, numRotations: rotationSolution, currentNumRotations: model.pentominoesArray[loopCounter].numRotations)
//                //                }else {
...
/*
        ...
        for aView in pentominoImageViews {
            ...
            self.weirdrotatePentominoView(aView, numRotations: rotationsNeeded, width: &pieceWidth, height: &pieceHeight, isSolve: self.isSolve, directionFactor: 1.0)
            self.model.pentominoesArray[loopCounter].numRotations += rotationsNeeded
            
            if flipsNeeded != 0 {
                self.weirdflipPentominoView(aView, numRotations: rotationsNeeded, x: negativeTransformValue, y: positiveTransformValue)
...
///*
        ...
        for aView in pentominoImageViews {
            ...
            UIView.animateWithDuration(rotationDuration, animations: { () -> Void in
                ...
                if flipSolution != 0 {
                    self.weirdflipPentominoView(aView, numRotations: rotationsNeeded, x: self.negativeTransformValue, y: self.positiveTransformValue)
                    self.model.pentominoesArray[loopCounter].numFlips += flipsNeeded
                }
...
/*
        if let tappedImageView = recognizer.view as? UIImageView {
            ...
            if isUserFlipped {
                weirdrotatePentominoView(tappedImageView, numRotations: 1, width: &width, height: &height, isSolve: isSolve, directionFactor: -1.0)
            }else {
                weirdrotatePentominoView(tappedImageView, numRotations: 1, width: &width, height: &height, isSolve: self.isSolve, directionFactor: 1.0)
            }
...
//*
        if let tappedImageView = recognizer.view as? UIImageView {
            ...
            if isUserFlipped {
                rotatePentominoView(tappedImageView, numRotations: 1, width: &width, height: &height, isSolve: isSolve)
            }else {
...
/*
        if let tappedImageView = recognizer.view as? UIImageView {
            ...
            if evenOrOdd == isOdd {
                weirdflipPentominoView(tappedImageView, numRotations: model.pentominoesArray[tappedImageView.tag].numRotations, x: self.negativeTransformValue, y: self.positiveTransformValue)
            } else {
                weirdflipPentominoView(tappedImageView, numRotations: model.pentominoesArray[tappedImageView.tag].numRotations, x: self.negativeTransformValue, y: self.positiveTransformValue)
            }
...
//*
        if let tappedImageView = recognizer.view as? UIImageView {
            let evenOrOdd = self.checkNumberOfRotations(model.pentominoesArray[tappedImageView.tag].numRotations )
            if evenOrOdd == isOdd {
                flipPentominoView(tappedImageView, numRotations: model.pentominoesArray[tappedImageView.tag].numRotations, currentNumRotations: model.pentominoesArray[tappedImageView.tag].numRotations)
            } else {
                flipPentominoView(tappedImageView, numRotations: model.pentominoesArray[tappedImageView.tag].numRotations, currentNumRotations: model.pentominoesArray[tappedImageView.tag].numRotations)
            }
    ...
    func findViewIndex(currentView :UIImageView) -> Int{
        ...
        for i in 0..<model.numPentominoesPieces {
            ...
            if currentView.letter == String(model.pentominoesArray[i].letter){
                //THIS STATEMENT NEVER GETS REACHED
                index = i
            }
        }
        return index
