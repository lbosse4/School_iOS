...
//  Instantiated by Lauren Bosse on 11/11/15.
...
class DataManager {
    ...
    private lazy var managedObjectModel: NSManagedObjectModel = {
        ...
        let modelURL = NSBundle.mainBundle().URLForResource(self.delegate!.xcDataModelName(), withExtension: "momd")!
    ...
    private  func databasePath() -> NSURL {
        let sqlite = delegate!.xcDataModelName() + ".sqlite"
        let path = self.applicationDocumentsDirectory.URLByAppendingPathComponent(sqlite)
        return path
        
    }
    
    private func databaseExists() -> Bool {
        let exists = NSFileManager.defaultManager().fileExistsAtPath(databasePath().path!)
        return exists
    }
